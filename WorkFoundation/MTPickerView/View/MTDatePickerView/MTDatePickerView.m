...
@property (nonatomic, copy) void (^finishBlock) (DateCompentObject *timeObject);
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.finishSelect = ^(NSArray <NSNumber *> *selectIndex){
            if (weakSelf.finishBlock && [selectIndex isExist]) {
                ...
                DateCompentObject *timeObject = [[DateCompentObject alloc] init];
...
- (void)prepareDate{
    MTWeakSelf;
    [[DateCompent sharedInstance] dayOffset:0 rezult:^(DateCompentObject *dateObject) {
        ...
        DateCompentObject *year = [DateCompentObject new];
 ...
 *  获取year的array
 **/
- (NSMutableArray <ItemModel *> *)getYearNextArrayWithStartYear:(DateCompentObject *)timeObject offset:(NSInteger)distance{
    ...
    for (NSInteger year = startYear; year < startYear + distance; year ++) {
        ...
        DateCompentObject *yearObject = [DateCompentObject new];
 ...
 *  根据年获取月的array
 **/
- (NSMutableArray <ItemModel *> *)getMonthWith:(DateCompentObject *)timeObject{
    ...
    for (NSInteger month = startMonth; month < 13; month ++) {
        ...
        DateCompentObject *yearMonthObject = [DateCompentObject new];
 ...
 *  根据年,月获取天的array
 **/
- (NSMutableArray <ItemModel *> *)getDayWith:(DateCompentObject *)timeObject{
    NSInteger startDay;
    NSInteger year = timeObject.year.integerValue;
    ...
    for (NSInteger day = startDay; day < fullDay; day ++) {
        ...
        DateCompentObject *yearMonthDayObject = [DateCompentObject new];
        yearMonthDayObject.year = @(year);
        yearMonthDayObject.month = @(month);
        yearMonthDayObject.day = @(day);
 ...
 *  根据年,月,天获取小时的array
 **/
- (NSMutableArray <ItemModel *> *)getHourWith:(DateCompentObject *)timeObject{
    NSInteger year = timeObject.year.integerValue;
    NSInteger month = timeObject.month.integerValue;
    ...
    for (NSInteger hour = startHour; hour < 24; hour ++) {
        ...
        DateCompentObject *yearMonthDayHourObject = [DateCompentObject new];
 ...
 **/
- (NSMutableArray <ItemModel *> *)getMinWith:(DateCompentObject *)timeObject{
    NSInteger year = timeObject.year.integerValue;
    NSInteger month = timeObject.month.integerValue;
    NSInteger day = timeObject.day.integerValue;
