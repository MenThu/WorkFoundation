...
- (void)share{
    MTWeakSelf;
    NSMutableDictionary *shareInfo = self.shareInfo.mutableCopy;
    if (_shareInfo[imgKey] && ![_shareInfo[imgKey] isKindOfClass:[NSArray class]]) {
        shareInfo[imgKey] = [NSArray arrayWithObject:_shareInfo[imgKey]];
    }
    [self pause];
    [[ShareTool sharedInstance] shareWithInfo:shareInfo finish:^(BOOL isSuccess){
        if (isSuccess) {
            [weakSelf startPlay];
            HttpRequest *request = [HttpRequest requestWithPath:@"userVoucher/addVoucher"];
            request.params = @{};
            [[HttpClient sharedInstance] post:request blockView:self finish:^(HttpResponse *response) {
                if (response.success) {
                    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self animated:YES];
                    hud.mode = MBProgressHUDModeText;
                    hud.labelText = @"赠送代金券成功";
