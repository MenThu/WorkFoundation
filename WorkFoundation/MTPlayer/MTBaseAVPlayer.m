...
- (void)prepareTimer{
    MTWeakSelf;
    CGFloat timeInterval = 0.05;
    self->_progressTimer = [MyTimer customTimeInterval:timeInterval WithBlock:^(NSInteger timeCount) {
        NSInteger currentTime = (NSInteger)CMTimeGetSeconds([weakSelf.avPlayerItem currentTime]);
//        CGFloat currentTime = weakSelf.avPlayerItem.currentTime.value / weakSelf.avPlayerItem.currentTime.timescale;
//        CGFloat currentTime = timeInterval * timeCount;
