 ...
 *  系统的当前时间
 **/
@property (nonatomic, strong) MTTimeObject *currentTime;
...
- (void)configView{
    ...
    monthView.dequeCell = ^(UICollectionView *collectionView, NSIndexPath *indexPath){
        CalendarMonthCell *monthCell = [collectionView dequeueReusableCellWithReuseIdentifier:mtBaseCollectionViewCell forIndexPath:indexPath];
    ...
    monthView.scrollerBlock = ^(CGPoint contentOffset){
        NSInteger currentPage = (NSInteger)(contentOffset.x / weakSelf.monthView.width + 0.5);
        [weakSelf changeTitleWith:currentPage];
    };
...
- (NSArray <CalendarMonth *> *)prepareDataForMonthView{
    ...
    [[MTTimerManager sharedInstance] secondOffsetFromNow:0 rezult:^(MTTimeObject *dateObject) {
